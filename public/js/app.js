...
//we use the Fetch API to make server requests (to JSON endpoint) from client side scripts using async and Ajax techniques.
// Fetch is  part of JS living standards (no specific version and features are available in Browsers as soon as
// they are implemented and do not need to wait to implement all features of a specific ECMA version)
...
//in this example using Fetch API , we make a request to a json endpoints and instead of returning a HTML blob from
// the server (from app.js Node file), now you return JSON, and Fetch will read JSON on client and allow to display
// the JSON values returned on the page. Compared with HBS which replaces the params passed by app.js Node file on server side
